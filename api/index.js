...
app.get('/api/health', async (req, res) => {
  ...
  res.json({ status: 'active', server: 'Kvantum Engine v4', databaseConnected: dbConnected });
});
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    const id = 'lead_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
    const msg = message || notes || 'Inquiry';
    const lead = { id, _id: id, name, email, phone: phone || '', service: service || 'Inquiry', message: msg, notes: msg, status: 'New', quality: 'Warm', createdAt: new Date().toISOString(), created_at: new Date().toISOString() };
    try {
      await db.query(
        `INSERT INTO leads ("_id","name","email","phone","service","message","notes","status","quality") VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
        [id, name || 'Client', email || '', phone || '', service || 'Inquiry', msg, msg, 'New', 'Warm']
      );
    } catch (dbErr) {
      console.warn('[LEAD DB WARN]', dbErr.message);
...
app.get('/api/leads', auth, async (req, res) => {
  try {
    ...
    const dbLeads = result.rows.map(r => ({
      id: r._id, _id: r._id, name: r.name, email: r.email, phone: r.phone,
      service: r.service, message: r.message, notes: r.notes || r.message,
...
app.delete('/api/leads/:id', auth, async (req, res) => {
  try {
    ...
    localLeads = localLeads.filter(l => l.id !== req.params.id);
  ...
  try {
    ...
    const assets = (data.Contents || []).map(item => {
      ...
      return {
        ...
        publicUrl: `${SUPABASE_URL}/${S3_BUCKET}/${item.Key}`,
        url: `${SUPABASE_URL}/${S3_BUCKET}/${item.Key}`,
        size: item.Size || 0,
        contentType: imgs.includes(ext) ? `image/${ext === 'jpg' ? 'jpeg' : ext}` : 'application/octet-stream'
      };
    });
    res.json([...localAssets, ...assets]);
  } catch (err) {
    console.warn('[S3 LIST]', err.message);
...
app.post('/api/assets/upload', auth, upload.single('file'), async (req, res) => {
  ...
  const asset = {
    ...
    publicUrl: `${SUPABASE_URL}/${S3_BUCKET}/${fileName}`,
    url: `${SUPABASE_URL}/${S3_BUCKET}/${fileName}`,
    size: req.file.size,
  ...
  try {
    ...
    console.warn('[S3 UPLOAD WARN]', err.message);
...
app.post('/api/users/register', auth, async (req, res) => {
  try {
    ...
    res.status(201).json({ id, _id: id, name, email, role: role || 'seo' });
...
app.put('/api/users/:id', auth, async (req, res) => {
  try {
    ...
    await db.query(`UPDATE users SET "name"=COALESCE($1,"name"),"role"=COALESCE($2,"role"),"updated_at"=NOW() WHERE "_id"=$3`, [name, role, req.params.id]);
    res.json({ id: req.params.id, ...req.body });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/services/:id', auth, async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [title, shortDesc, longDesc, metaTitle, metaDesc, keywords, coverImage, icon, status, req.params.id]
    );
...
app.put('/api/blogs/:id', auth, async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [title, excerpt, content, metaTitle, metaDesc, coverImage, tags, author, status, req.params.id]
    );
    res.json({ id: req.params.id, _id: req.params.id, ...req.body });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/seosettings', auth, async (req, res) => {
  try {
    const r = await db.query('SELECT * FROM seo_settings');
    const obj = {};
...
app.put('/api/seosettings/:key', auth, async (req, res) => {
  try {
    ...
    await db.query(
      `INSERT INTO seo_settings ("key","title","description","keywords","schema","other","content","updated_at") VALUES ($1,$2,$3,$4,$5,$6,$7,NOW()) ON CONFLICT ("key") DO UPDATE SET "title"=EXCLUDED."title","description"=EXCLUDED."description","keywords"=EXCLUDED."keywords","schema"=EXCLUDED."schema","other"=EXCLUDED."other","content"=EXCLUDED."content","updated_at"=NOW()`,
...
app.put('/api/seosettings/canonical', auth, async (req, res) => {
  try {
    const { url } = req.body;
    await db.query(`INSERT INTO seo_settings ("key","content","updated_at") VALUES ('canonical',$1,NOW()) ON CONFLICT ("key") DO UPDATE SET "content"=$1,"updated_at"=NOW()`, [url]);
...
function getMeta(pathname) {
  if (metaMap[pathname]) return metaMap[pathname];
...
export default async function handler(req, res) {
  try {
    ...
    if (pathname === '/robots.txt') {
      ...
      try {
        const r = await db.query('SELECT "content" FROM seo_settings WHERE "key"=$1', ['robots']);
        const content = r.rows[0]?.content;
        if (content) return res.status(200).send(content);
      } catch {}
      try {
        return res.status(200).send(fs.readFileSync(path.join(process.cwd(), 'public', 'robots.txt'), 'utf8'));
      } catch {
    ...
    if (pathname === '/sitemap.xml') {
      ...
      res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
      try {
        const r = await db.query('SELECT "content" FROM seo_settings WHERE "key"=$1', ['sitemap']);
        const content = r.rows[0]?.content;
        if (content) return res.status(200).send(content);
      } catch {}
      try {
        return res.status(200).send(fs.readFileSync(path.join(process.cwd(), 'public', 'sitemap.xml'), 'utf8'));
      } catch {
    ...
    const { title, desc } = getMeta(pathname);
