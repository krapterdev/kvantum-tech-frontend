...
export default function About({ settings }) {
  ...
  return (
    <section className="bg-slate-50 dark:bg-zinc-950/40 border-y border-slate-200 dark:border-white/8 py-24 px-6 select-none">
      <div className="container mx-auto max-w-[1280px]">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center text-left">
          ...
          <div className="flex flex-col items-start">
            ...
            <h2 className="text-3xl sm:text-4xl font-headline font-bold text-slate-900 dark:text-white mb-4 leading-tight">
              Software Built Around Your Business
            </h2>

            <p className="text-sky-600 dark:text-cyanCustom font-semibold text-base sm:text-lg mb-4">
              Every business operates differently. Your software should too.
            </p>

            <div className="text-slate-600 dark:text-zinc-300 text-sm sm:text-base leading-relaxed flex flex-col gap-4 mb-6">
              <p>
              ...
              <p className="text-slate-900 dark:text-zinc-200 font-medium">
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {highlights.map((h, idx) => (
                <div key={idx} className={`flex items-center gap-1.5 text-xs font-mono px-3 py-1.5 rounded-lg bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/8 ${h.color}`}>
                  <CheckCircle2 size={12} />
                  <span className="text-slate-800 dark:text-zinc-200">{h.label}</span>
                </div>
              ))}
            </div>
            ...
            <button
              ...
              className="group text-sm font-bold text-sky-600 dark:text-cyanCustom hover:text-sky-700 dark:hover:text-cyan-400 flex items-center gap-1.5 cursor-pointer transition-colors duration-200"
            >
          ...
          <div className="flex justify-center w-full">
            <div className="relative w-full max-w-[440px]">
              ...
              <Card className="w-full p-8 border border-slate-200 dark:border-white/12 bg-white dark:bg-zinc-900/90 flex flex-col gap-5 relative overflow-hidden shadow-xl dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
                ...
                {[
                  ...
                  {
                    icon: Code,
                    iconClass: 'text-sky-600 dark:text-cyanCustom bg-sky-500/10 border-sky-500/20',
                    badge: 'Technology',
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="text-[10px] font-mono text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">{item.badge}</span>
                        <h4 className="text-slate-900 dark:text-zinc-100 font-semibold text-sm font-headline leading-snug">{item.title}</h4>
                      </div>
                    </div>
                  );
                })}

                <div className="border-t border-slate-200 dark:border-white/8 pt-4 mt-1 flex justify-between items-center text-xs font-mono text-slate-500 dark:text-zinc-500">
